...
An identifier that has been defined as a base quantity may not be
...
\begin{syntax}
\key{unit} \id{1} \key{for} \id{2}
\end{syntax}
where \id{2} is a previously-defined quantity and \id{1} is the name
of the new unit.
...
\key{in} operator (Section~\ref{in-op}).
...
\end{syntax}
where \expr{1} is an expression that evaluates to a number or vector
with units, and \expr{2} evaluates to units.  The \key{in} operator
...
units.  It is an error if \expr{1} and \expr{2} do not have the same
quantity, or if a valid conversion factor between the units of
\expr{1} and the units of \expr{2} cannot be found.
...
point of the \key{load} statement.  Any definitions in the loaded file
...
An if/then/else block begins with the keyword \key{if}, followed by an
...
The \key{print()} function takes a single argument, which may be any
...
The \key{stripUnits()} function takes an argument of a number with
...
The \key{toPrecision()} function takes two arguments, a number
...
The \key{toInt()} rounds its argument to the nearest integer, using
the same ``round-to-even'' method as \key{toPrecision()}.  If the
number includes units, those units are preserved in the result.

\subsection{read()}

The \key{read()} function takes no arguments.  It reads a single value
...
The \key{readline()} function, which takes no arguments, reads a line
