...
public class PNumber extends Datum  {
    ...
    public Datum div(Datum that) throws TypeError {
		if (that instanceof PNumber) {
			return new PNumber((numValue.divide(((PNumber)that).numValue,20,java.math.BigDecimal.ROUND_HALF_EVEN)).toString());
		} else if (that instanceof PUnitPair) {
			return new PUnitPair(this.div(((PUnitPair)that).getNumber()),((PUnitPair)that).getUnit().neg());
		} else {
			throw new TypeError("/", this, that);
		}
    }
