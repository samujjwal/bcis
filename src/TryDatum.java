    ...
    public static void main(String [] args) {
		...
		*****************/
		...
		try {
			...
			PUnitPair accel = num1.mul(foot.div(minute.pow(new PNumber("2"))));  // accel = 0.5 * foot / minute^2 = 0.0000423 * meter / second^2
			System.out.println("accel: "+accel.toString());

			PUnitPair time = num2.mul(second);  // time = 30 * second
			System.out.println("time: "+time.toString());

			PUnitPair veloc = time.mul(accel);  // veloc = time * accel = 30 * second * 0.0000423 * meter / second^2 = 0.0762 * meter / second
